...
// ARTICULACION LLANTA 4
		...
		//AQUÍ SE DIBUJA LA CABINA
		model = glm::translate(model, glm::vec3(2.0f, 2.8f, 2.0f));
		//rotación alrededor de la articulación que une con la cabina
		modelaux = model;
		model = glm::scale(model, glm::vec3(4.0f, 2.2f, 2.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = modelaux;
		...
		// Se crea la cabina 
		model = glm::translate(model, glm::vec3(-1.f, 1.5f, 0.0f));
		//model = glm::rotate(model, glm::radians(-135.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.8f, 3.0f, 2.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.52f, 0.2f, 0.79f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh();
